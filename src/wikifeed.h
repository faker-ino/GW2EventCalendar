...
// the same bonus-event rotations as the forum's ICS calendar feed
// (icsfeed.h), but - the whole reason this parses the rendered HTML instead
// of raw wikitext - adds a Description/Features/Bonus-effects breakdown per
// event that the ICS feed has no equivalent of at all. See
// EventStore::FetchThreadMain in eventfetcher.cpp for how this gets merged
// with (and used to enrich) the ICS-sourced events.
//
